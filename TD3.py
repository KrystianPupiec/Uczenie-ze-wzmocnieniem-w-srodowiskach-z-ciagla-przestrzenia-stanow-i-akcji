...
class TD3:
    ...
    def train(self, replay_buffer, batch_size=128):
        ...
        with torch.no_grad():
            # Wygładzanie polityki docelowej
        ...
        # Opóźniona aktualizacja polityki
...
def train_td3(seed, batch_size=64, episodes=10001, record_video=False, noise_type='gauss', reward_scale=0.1, learning_starts=2000):
    ...
    # Przekazanie wymiarów do nowego bufora pamięci
...
def train_sb3_td3(seed, episodes=10001):
    ...
    # Bez włączania renderowania graficznego
    env = gym.make("BipedalWalker-v3")
    env = Monitor(env)
    env.reset(seed=seed)

    # ustawienie argumentu `device='cpu'` dla SB3
    model = SB3_TD3("MlpPolicy", env, seed=seed, learning_rate=1e-4, gamma=0.99, tau=0.001, batch_size=128, device='cpu')
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        przyszle_seedy = {executor.submit(train_td3, seed, domyslny_batch, liczba_epizodow, seed == testowy_seed): seed for seed in seedy}
        ...
        for future in concurrent.futures.as_completed(przyszle_seedy):
            ...
            # Zapis tymczasowy
            np.savez(f'temp_1a_seed_{seed}_td3.npz', nagrody=nagrody, straty=straty, kroki=kroki)
    ...
    np.savez('dane_seedy_td3.npz', **wyniki_seedy_nagrody)
    np.savez('dane_seedy_straty_td3.npz', **wyniki_seedy_straty)
    np.savez('dane_seedy_kroki_td3.npz', **wyniki_seedy_kroki) 
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        przyszle_seedy_ou = {
            executor.submit(train_td3, seed, domyslny_batch, liczba_epizodow, False, 'ou'): seed for seed in seedy}
        ...
        for future in concurrent.futures.as_completed(przyszle_seedy_ou):
            ...
            # Zapis tymczasowy
            np.savez(f'temp_1b_seed_{seed}_ou_td3.npz', nagrody=nagrody, straty=straty, kroki=kroki) 
    ...
    np.savez('dane_seedy_ou_td3.npz', **wyniki_seedy_nagrody_ou)
    np.savez('dane_seedy_straty_ou_td3.npz', **wyniki_seedy_straty_ou)
    np.savez('dane_seedy_kroki_ou_td3.npz', **wyniki_seedy_kroki_ou) 


    print("2. Zbieranie danych dla różnych batch_size")
    testowane_batche = [32, 64, 128, 256, 512]
    wyniki_batche = {}
    wyniki_batche_straty = {}
    wyniki_batche_kroki = {} 
    
    with concurrent.futures.ProcessPoolExecutor() as executor:
        przyszle_batche = {executor.submit(train_td3, testowy_seed, bs, liczba_epizodow, False): bs for bs in testowane_batche}
        ...
        for future in concurrent.futures.as_completed(przyszle_batche):
            ...
            # Zapis tymczasowy
            np.savez(f'temp_2_batch_{bs}_td3.npz', nagrody=nagrody, straty=straty, kroki=kroki) 
            
            wyniki_batche[f"Batch {bs}"] = nagrody
            wyniki_batche_straty[f"Batch {bs}"] = straty
            wyniki_batche_kroki[f"Batch {bs}"] = kroki
            print(f"Ukończono batch {bs}")
            
    np.savez('dane_batche_td3.npz', **wyniki_batche)
    np.savez('dane_batche_straty_td3.npz', **wyniki_batche_straty)
    np.savez('dane_batche_kroki_td3.npz', **wyniki_batche_kroki) 
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        przyszle_szumy = {executor.submit(train_td3, testowy_seed, domyslny_batch, liczba_epizodow, False, szum): szum for szum in rodzaje_szumu}
        ...
        for future in concurrent.futures.as_completed(przyszle_szumy):
            ...
            # Zapis tymczasowy
            np.savez(f'temp_3_szum_{szum}_td3.npz', nagrody=nagrody, straty=straty, kroki=kroki) 
            
            wyniki_szumy[f"Szum {szum}"] = nagrody
            wyniki_szumy_straty[f"Szum {szum}"] = straty
            wyniki_szumy_kroki[f"Szum {szum}"] = kroki
            print(f"Ukończono szum {szum}")
            
    np.savez('dane_szumy_td3.npz', **wyniki_szumy)
    np.savez('dane_szumy_straty_td3.npz', **wyniki_szumy_straty)
    np.savez('dane_szumy_kroki_td3.npz', **wyniki_szumy_kroki) 
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        przyszle_skale = {executor.submit(train_td3, testowy_seed, domyslny_batch, liczba_epizodow, False, 'gauss', skala): skala for skala in skale_nagrody}
        ...
        for future in concurrent.futures.as_completed(przyszle_skale):
            ...
            # Zapis tymczasowy
            np.savez(f'temp_4_skala_{skala}_td3.npz', nagrody=nagrody, straty=straty, kroki=kroki) 
    ...
    np.savez('dane_skale_td3.npz', **wyniki_skale)
    np.savez('dane_skale_straty_td3.npz', **wyniki_skale_straty)
    np.savez('dane_skale_kroki_td3.npz', **wyniki_skale_kroki) 


    print("5. Zbieranie danych ze Stable-Baselines3")
    sb3_nagrody, sb3_kroki = train_sb3_td3(testowy_seed, episodes=liczba_epizodow)
    
    # Zapis tymczasowy dla SB3
    np.savez('temp_5_sb3_td3.npz', nagrody=sb3_nagrody, kroki=sb3_kroki) 
    
    wyniki_sb3 = {
        "Własna implementacja TD3": wyniki_seedy_nagrody[f"Seed {testowy_seed}"],
        "SB3 TD3": sb3_nagrody
    }
    wyniki_sb3_kroki = {
        "Własna implementacja TD3": wyniki_seedy_kroki[f"Seed {testowy_seed}"],
        "SB3 TD3": sb3_kroki
    }
    np.savez('dane_sb3_td3.npz', **wyniki_sb3)
    np.savez('dane_sb3_kroki_td3.npz', **wyniki_sb3_kroki) 
    print("Ukończono SB3")


    print("6. Zbieranie danych dla różnych szumów bez rozgrzewki")
    wyniki_szumy_bez_rozgrzewki = {}
    wyniki_szumy_bez_rozgrzewki_straty = {}
    wyniki_szumy_bez_rozgrzewki_kroki = {} 
    
    with concurrent.futures.ProcessPoolExecutor() as executor:
        przyszle_bez_rozgrzewki = {executor.submit(train_td3, testowy_seed, domyslny_batch, liczba_epizodow, False, szum, 0.1, 0): szum for szum in rodzaje_szumu}
        ...
        for future in concurrent.futures.as_completed(przyszle_bez_rozgrzewki):
            ...
            # Zapis tymczasowy
            np.savez(f'temp_6_bezrozgrzewki_szum_{szum}_td3.npz', nagrody=nagrody, straty=straty, kroki=kroki) 
            
            wyniki_szumy_bez_rozgrzewki[f"Szum {szum}"] = nagrody
            wyniki_szumy_bez_rozgrzewki_straty[f"Szum {szum}"] = straty
            wyniki_szumy_bez_rozgrzewki_kroki[f"Szum {szum}"] = kroki
            print(f"Ukończono szum {szum} bez rozgrzewki")
            
    np.savez('dane_szumy_bez_rozgrzewki_td3.npz', **wyniki_szumy_bez_rozgrzewki)
    np.savez('dane_szumy_bez_rozgrzewki_straty_td3.npz', **wyniki_szumy_bez_rozgrzewki_straty)
    np.savez('dane_szumy_bez_rozgrzewki_kroki_td3.npz', **wyniki_szumy_bez_rozgrzewki_kroki) 
